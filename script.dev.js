...
// ========== وحدة حماية الكود والملكية الفكرية (Code Protection & Anti-DevTools) ==========
(function initCodeProtection() {
    // منع قائمة الفأرة الأيمن (يُستثنى منها حقول الإدخال لتسهيل تجربة الاستخدام)
    ...
    // منع اختصارات لوحة المفاتيح الشائعة لفتح أدوات التطوير وقراءة السورس
    document.addEventListener('keydown', function (e) {
        ...
        // Ctrl+Shift+I (Inspect), Ctrl+Shift+J (Console), Ctrl+Shift+C (Element picker), Ctrl+U (View Source)
        if (e.ctrlKey && (
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ORDER_COOLDOWN_MS = 10000; // 10 ثوانٍ كحد أدنى بين الطلبات المتتالية
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', (e) => {
            ...
            // 1. فحص الحقل الخفي (Honeypot) - لو تم ملئه بواسطة بوت يتوقف الإرسال فوراً
            const hpValue = document.getElementById("website_hp_confirm")?.value;
            if (hpValue && hpValue.trim() !== '') {
                console.warn('Bot submission blocked via Honeypot.');
                return;
            }

            // 2. فحص تقييد المعدل الزمني (Rate Limiting Cooldown)
            const nowTime = Date.now();
            if (nowTime - lastOrderTimestamp < ORDER_COOLDOWN_MS) {
                alert(currentLang === 'ar' 
                    ? 'برجاء الانتظار عدة ثواني قبل إرسال طلب جديد.' 
            ...
            // تنقية المدخلات لمنع الثغرات
            email = escapeHTML(email.trim());
            name = escapeHTML(name.trim());
            governorate = escapeHTML(governorate.trim());
            address = escapeHTML(address.trim());
            phone = phone.trim();

            if (!phone || !/^[0-9+\s\-]{8,15}$/.test(phone)) {
                alert(currentLang === 'ar' ? 'برجاء إدخال رقم هاتف صحيح.' : 'Please enter a valid phone number.');
            ...
            const transactionNumber = escapeHTML((document.getElementById("transactionNumber")?.value || '').trim());
            ...
            const productsString = cart.map(item => {
                ...
                return escapeHTML(itemName) + ' (x' + item.quantity + ')';
            }).join('\n');

            const finalTotal = escapeHTML(document.getElementById("checkoutFinalTotal") ? document.getElementById("checkoutFinalTotal").textContent : '0');
